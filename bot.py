...
TG_CHAT_ID = os.environ.get("TG_CHAT_ID", "-1003890109420").strip()
# Хранилище состояния: файл в репозитории. GitHub Actions коммитит его обратно после прогона.
...
# (Периодичность теперь задаётся в Railway Cron, а не здесь.)
...
- РОЛИ РОСТА (тоже в цель, это логичный следующий шаг Димы — оценивай высоко):
  Senior / Lead / Executive Producer; Delivery Lead / Head of Delivery / Head of Production;
  Program Director / Portfolio Manager; Client Services Director / Group Account Director /
  Client Partner; Studio / Resource / Operations Manager в креативе-диджитале.
- Целевые индустрии: creative / branding agency, digital agency, fintech, SaaS,
  media, product-компании с дизайн-командами.
  ВАЖНО: продуктовая компания — ОК как индустрия, если РОЛЬ продюсерская/проектная
  (напр. Creative/Game Producer в продуктовой компании — подходит). Но роль
  Product Manager / Product Owner в такой компании — НЕ подходит (см. ниже).
...
- Языки: русский (родной) + English B2 (FCE), может собеседоваться голосом.
  Если жёстко требуется C1/native English — риск: понижай балл, но не стоп.
  Если ОБЯЗАТЕЛЕН другой язык (немецкий, французский, испанский и т.п.) на уровне
  C1/native — сильный минус, почти стоп (этих языков у Димы нет).
...
- НЕ ЕГО ПРОФИЛЬ (ставь 1-3, ядро роли — чужая ему специализация):
  • Product Manager / Product Owner в IT/tech/SaaS (владение продуктом, roadmap, discovery,
    продуктовые метрики, работа с инженерами над софтом) — Дима ПРОДЮСЕР/ПРОЕКТНЫЙ, не продакт;
  • Аналитика / данные / CRO / веб-аналитика, где ядро — GA4, SQL, BigQuery, A/B-тесты, дашборды;
  • Разработка / QA / DevOps / дизайн-исполнение и прочие роли, требующие хард-скиллов,
    которых у Димы нет.
- ГЛАВНЫЙ ПРИНЦИП: оценивай по ЯДРУ роли и совпадению с реальным опытом Димы
  (агентство: управление проектами, продакшн, delivery, клиенты/аккаунт, presale, экономика
  проектов). НЕ ставь высокий балл только из-за слов "Manager", "remote" или хорошей зарплаты,
  если ядро роли — не его специализация.
...
def fetch_tg_channel(channel):
    """Читает публичную веб-страницу t.me/s/<channel> через BeautifulSoup.
    У любого публичного канала эта страница есть по умолчанию (превью Telegram, без логина)."""
    ...
    try:
        ...
        for m in messages[-12:]:  # последние ~12 сообщений
            ...
            post = m.get("data-post", "")            # вида "channel/1234"
...
if __name__ == "__main__":
    # Разовый запуск: один прогон и выход. Периодичность задаёт Railway Cron.
    try:
        run()
    except QuotaExhausted as e:
        print(f"Лимит Gemini исчерпан ({e}). Завершаюсь, повторю по следующему расписанию.")
    except Exception as e:
